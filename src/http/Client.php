 ...
 *
 *  <code>
 *     $client = new \PG\paySDK\http\Client();
...
class Client {
    /**
     * @todo wrap this in Request object
     * @var array of HTTP request headers
     */
    private $headers;

    /**
     * @todo wrap this in Response object
     * @var array of headers from HTTP response
     */
    ...
    /**
     * @todo wrap this Response object
     * @var array of headers from HTTP response
     */
    ...
    private function checkResponse($ch)
    {
        ...
        if ($this->failOnError) {
            ...
            if ($status >= 400 && $status <= 499) {
                throw new ClientError($this->getStatusMessage(), $status, $this->getResponse());
            } elseif ($status >= 500 && $status <= 599) {
                throw new ServerError($this->getStatusMessage(), $status, $this->getResponse());
            }
        }
     ...
     *
     * @param string $uri URI address to request
     * @param mixed $data querystring parameters
     * @param $data
     * @return $this
     * @throws ClientError
     * @throws NetworkError
    ...
    {
        return new Response($this->getStatus(), $this->getHeaders(), $this->getBody());
    }
